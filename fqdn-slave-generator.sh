...
while read k; do
  ...
  if [[ -n $currentZone ]] && [[ $zoneType == 'slave;' ]] && [[ $k == *'masters'* ]]
    then
    slaveZoneMaster=`awk '{print $3}' <<< "$k"`
    slavezoneMaster="${slaveZoneMaster%%\;/}"
    slaveZoneMaster="${slaveZoneMaster##*\ /}"
    slaveZoneMaster="${slaveZoneMaster//\;*/}"
#echo $slaveZoneMaster
#echo $zoneType
#echo $currentZone
    processZone $slaveZoneMaster

  fi

  if [[ -n $currentZone ]] && [[ $zoneType == 'slave;' ]] && [[ mastersNextLine == true ]]
    then
    zoneFile=`awk '{print $3}' <<< "$k"`
    zoneFile="${zoneFile##*\/}"
    zoneFile="${zoneFile/\"\;/}"
...
if [[ $zoneType == 'slave;' ]]
then
  zoneContent=`dig @$1 $currentZone axfr`
	echo $zoneContent
fi  
	...
	if [[ $z == *[0-9]*')'* ]] || [[ $zoneType == 'slave;' ]]
	  ...
	  if [[ $z == *'TXT'* ]] || [[ $z == *'SPF'* ]] || [[ $z == *'MX'* ]] || [[ $z == *'NS'* ]] || [[ $z == *'SRV'* ]] || [[ $z == *'LOC'* ]] || [[ $z == *'RRSIG'* ]] || [[ $z == *'DNSKEY'* ]]
	    then
	  ...
	  case $currentRecord in
	    ...
	    *'.') 
		...
		elif [[ $recordType == 'RRSIG' ]] || [[ $recordType == 'DNSKEY' ]]
		then
			echo '' > /dev/null
		else
			echo '' > /dev/null
