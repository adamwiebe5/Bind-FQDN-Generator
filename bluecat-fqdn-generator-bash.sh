...
if [[ $z == '$INCLUDE'* ]]
  then
  includeFQDN=`awk '{print $3}' <<< "$z"`
  echo '$ORIGIN '"$includeFQDN" >> ~/temp/fqdn-out/$zoneFile
  else
  echo "$z" >> ~/temp/fqdn-out/$zoneFile
fi
	...
	if [[ $z == *[0-9]*')'* ]]
	  ...
	  if [[ $z == '$INCLUDE'* ]]
	    then
	    includedFile=`awk '{print $2}' <<< "$z"`
	    currentZone=`awk '{print $3}' <<< "$z"`
	    currentZone=${currentZone%.*}
	    processZone $includedFile
	  fi
